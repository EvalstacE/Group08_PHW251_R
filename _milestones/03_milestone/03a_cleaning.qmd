...
pop_df_recat <- pop_df %>%
  mutate(new_age_group = 
          if_else(age_cat %in% c("0-4", "5-11", "12-17"),
              "0-17", age_cat)) %>%
...
We created two functions that 1) take a date field and create two new columns for the MMWR week and year, and 2) take the MMWR week and year and create two columns for the MMWR week start date and the MMWR end date:


Cleaning date fields and creating 4 new columns: MMWR week, MMWR year, MMWR week start date, and MMWR week end date:
...
Each of the three datasets code Race / Ethnicity differently. The Calfornia dataset uses a coded number, the LA County dataset uses a longer or full text, and the population dataset uses a shorthand text. 

We created a .csv that maps the codes, long text, and short text to each other: 
