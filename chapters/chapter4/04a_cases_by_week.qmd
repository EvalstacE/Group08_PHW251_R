...
cnty_week_data <- combined_df %>%
  select(health_officer_region, county, mmwr_year, mmwr_week, start_date, end_date,
         cumulative_infected, cumulative_severe, cumulative_unrecovered, 
         total_cnty_pop, total_HOR_pop, total_ca_pop)

ca_cnty_sf_week <- counties(state = "CA", cb = TRUE, year = 2023) %>%
  st_as_sf() %>%
  select(NAME) %>%
  left_join(cnty_week_data, by = c("NAME" = "county")) %>%
  rename("county" = "NAME") %>%
  st_transform(4326)
...
##-st_write(ca_cnty_sf_week, here("data/shapefiles/ca_cnty_sf_week/ca_cnty_sf_week.shp"))
